...
#align_import section04groups.sheet1
...
example (a b c : G) : a * b * c = a * (b * c) := by sorry

example (a : G) : a * 1 = a := by sorry

-- Can you guess the last two?
example (a : G) : 1 * a = a := by sorry

example (a : G) : a * a⁻¹ = 1 := by sorry
...
example : a⁻¹ * (a * b) = b := by sorry

example : a * (a⁻¹ * b) = b := by sorry

example {a b c : G} (h1 : b * a = 1) (h2 : a * c = 1) : b = c :=
  by-- hint for this one if you're doing it from first principles: `b * (a * c) = (b * a) * c`
  sorry
...
example (h : ∀ g : G, g * g = 1) : ∀ g h : G, g * h = h * g := by sorry
