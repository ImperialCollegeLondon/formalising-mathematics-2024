...
#align_import section12measure_theory.sheet2
...
-- to get notation `measurable_set[S] U` 
-- for "U is in the sigma algebra S"
example : MeasurableSpace X :=
  { MeasurableSet' := fun U => ∀ i : I, measurable_set[𝓐 i] U
    measurable_set_empty := by sorry
    measurable_set_compl := by sorry
    measurable_set_iUnion := by sorry }
