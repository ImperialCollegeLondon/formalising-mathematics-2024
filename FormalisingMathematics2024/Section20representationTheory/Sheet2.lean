...
#align_import section20representation_theory.sheet2
...
variable {k : Type} [Field k] {G : Type} [Group G] {V : Type} [AddCommGroup V] [Module k V]
  {W : Type} [AddCommGroup W] [Module k W] (ρ : Representation k G V) (σ : Representation k G W)
...
def id (ρ : Representation k G V) : RepMap ρ ρ
    where
...
def comp {τ : Representation k G X} (ψ : RepMap σ τ) (φ : RepMap ρ σ) : RepMap ρ τ
    where
  toLinearMap := ψ.toLinearMap.comp φ.toLinearMap
  map_apply := sorry

theorem comp_id (φ : RepMap ρ σ) : φ.comp (id ρ) = φ := by sorry

theorem id_comp (φ : RepMap ρ σ) : (id σ).comp φ = φ := by sorry

theorem comp_assoc {τ : Representation k G X} {Y : Type} [AddCommGroup Y] [Module k Y]
    {υ : Representation k G Y} (ξ : RepMap τ υ) (ψ : RepMap σ τ) (φ : RepMap ρ σ) :
    (ξ.comp ψ).comp φ = ξ.comp (ψ.comp φ) := by sorry
