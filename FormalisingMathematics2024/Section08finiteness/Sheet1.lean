...
#align_import section08finiteness.sheet1
...
example (X : Type) (S : Set X) (T : Set X) (hs : Set.Finite S) (ht : T.Finite) : (S ∪ T).Finite :=
  by-- you cannot hope to prove this from first principles unless you really know
...
-- enable ∑ notation
example : ∑ i in Finset.range 10, i = 45 := by rfl

-- the advantage of constructive finiteness is that Lean knows a complete list of 
...
example (n : ℕ) : ∑ i in Finset.range n, (i : ℚ) ^ 3 = n ^ 2 * (n - 1) ^ 2 / 4 := by sorry
