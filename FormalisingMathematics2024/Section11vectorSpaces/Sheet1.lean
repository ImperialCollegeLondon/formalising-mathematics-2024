...
#align_import section11vector_spaces.sheet1
...
-- `X ≤ Y` is a true-false statement
example : Subspace k V :=
  X ⊓ Y

-- intersection of `X` and `Y`, as a subspace
example : Subspace k V :=
  X ⊔ Y

-- `X` + `Y`, as a subspace. It's the smallest vector subspace of
...
-- the type of `v` is `V`, and the true-false statement is `v ∈ X`
...
example (X : Subspace k V) (Y : Subspace k W) : X.map φ ≤ Y ↔ X ≤ Y.comap φ := by sorry
