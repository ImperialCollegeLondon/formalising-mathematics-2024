...
#align_import section15number_theory.sheet3
...
theorem divides_of_cong_four (t : ℕ) :
    5 ∣ 4 * (65 * t + 4) ^ 2 + 1 ∧ 13 ∣ 4 * (65 * t + 4) ^ 2 + 1 := by sorry

-- There are arbitrarily large solutions to `5 ∣ 4*n^2+1 ∧ 13 ∣ 4*n^2+1`
theorem arb_large_soln : ∀ N : ℕ, ∃ n > N, 5 ∣ 4 * n ^ 2 + 1 ∧ 13 ∣ 4 * n ^ 2 + 1 := by sorry
...
-- way is `finset.sup`.
theorem infinite_iff_arb_large (S : Set ℕ) : S.Infinite ↔ ∀ N, ∃ n > N, n ∈ S := by sorry
...
theorem infinite_setOf_solutions : {n : ℕ | 5 ∣ 4 * n ^ 2 + 1 ∧ 13 ∣ 4 * n ^ 2 + 1}.Infinite :=
  by
