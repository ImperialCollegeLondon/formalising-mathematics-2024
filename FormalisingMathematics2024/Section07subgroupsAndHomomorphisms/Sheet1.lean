...
#align_import section07subgroups_and_homomorphisms.sheet1

-- imports all the Lean tactics
-- imports all the Lean tactics
-- imports the theory of subgroups
-- imports the theory of subgroups
...
example (ha : a ∈ H) (hb : b ∈ H) : a * b⁻¹ ∈ H := by sorry
...
-/
example : CompleteLattice (Subgroup G) :=
  inferInstance
...
theorem Conjugate.one_mem : (1 : G) ∈ {a : G | ∃ h, h ∈ H ∧ a = x * h * x⁻¹} := by sorry

theorem Conjugate.inv_mem (hy : y ∈ {a : G | ∃ h, h ∈ H ∧ a = x * h * x⁻¹}) :
    y⁻¹ ∈ {a : G | ∃ h, h ∈ H ∧ a = x * h * x⁻¹} := by sorry

theorem Conjugate.hMul_mem (hy : y ∈ {a : G | ∃ h, h ∈ H ∧ a = x * h * x⁻¹})
    (hz : z ∈ {a : G | ∃ h, h ∈ H ∧ a = x * h * x⁻¹}) :
    y * z ∈ {a : G | ∃ h, h ∈ H ∧ a = x * h * x⁻¹} := by sorry
...
theorem mem_conjugate_iff : a ∈ conjugate H x ↔ ∃ h, h ∈ H ∧ a = x * h * x⁻¹ :=
  by-- true by definition!
  rfl

theorem conjugate_mono (H K : Subgroup G) (h : H ≤ K) : conjugate H x ≤ conjugate K x :=
  by-- start with `intro g` because the goal is definitionally 
  -- `∀ g, g ∈ conjugate H x → g ∈ conjugate K x`
  sorry

theorem conjugate_bot : conjugate ⊥ x = ⊥ :=
  by-- recall that ⊥ is the trivial subgroup and I showed you the basic API for it above.
  -- Start this proof with `ext a`.
  sorry

theorem conjugate_top : conjugate ⊤ x = ⊤ := by sorry
