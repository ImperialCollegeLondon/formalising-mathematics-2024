...
example (k : Type) [Field k] (I : Type) : UniqueFactorizationMonoid (MvPolynomial I k) :=
  inferInstance
...
theorem Ideal.IsPrime.not_one_mem {R : Type} [CommRing R] {P : Ideal R} (hI : P.IsPrime) :
    (1 : R) ∉ P := by
  intro h
  apply hI.ne_top
  rwa [Ideal.eq_top_iff_one]

theorem Ideal.IsPrime.mem_of_prod_mem {R : Type} [CommRing R] {P : Ideal R} (hP : P.IsPrime)
    {L : Multiset R} : L.Prod ∈ P → ∃ x : R, x ∈ L ∧ x ∈ P :=
  by
...
theorem Prime.ideal_span_singleton_isPrime {R : Type} [CommRing R] {p : R} (hp : Prime p) :
    (Ideal.span {p} : Ideal R).IsPrime :=
  by
  rwa [Ideal.span_singleton_prime]
  exact hp.ne_zero

-- All height one primes are principal in a UFD.
example (R : Type) [CommRing R] [IsDomain R] [UniqueFactorizationMonoid R] (P : Ideal R) :
    IsHeightOnePrime P → P.IsPrincipal :=
  by
  rintro ⟨hPprime, hPnonzero, hht1⟩
  -- P is nonzero so choose nonzero x ∈ P
  have hnonzero : ∃ x ∈ P, x ≠ (0 : R) := by
    by_contra' h
    apply hPnonzero
    ext
  ...
  obtain ⟨L, hLprime, hLx⟩ := UniqueFactorizationMonoid.exists_prime_factors x hx0
  ...
  · -- and if not then pi is prime
    ...
    have hpi : (Ideal.span {pi}).IsPrime := hpiprime.ideal_span_singleton_is_prime
