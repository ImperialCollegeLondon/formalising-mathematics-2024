...
#align_import solutions.section02reals.sheet6
...
theorem tendsTo_hMul_const {a : ℕ → ℝ} {t : ℝ} (c : ℝ) (h : TendsTo a t) :
    TendsTo (fun n => a n * c) (t * c) := by simpa [mul_comm] using tends_to_const_mul c h
...
theorem tendsTo_of_tendsTo_sub {a b : ℕ → ℝ} {t u : ℝ} (h1 : TendsTo (fun n => a n - b n) t)
    (h2 : TendsTo b u) : TendsTo a (t + u) := by simpa using tends_to_add h1 h2
...
theorem tendsTo_zero_hMul_tendsTo_zero {a b : ℕ → ℝ} (ha : TendsTo a 0) (hb : TendsTo b 0) :
  ...
  simpa [abs_mul] using mul_lt_mul'' hX hY _ _ <;> exact abs_nonneg _
