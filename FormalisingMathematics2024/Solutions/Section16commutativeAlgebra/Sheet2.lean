...
#align_import solutions.section16commutative_algebra.sheet2
...
theorem lemma1 {A : Type} [CommRing A] [DecidableEq A] {I : Ideal A} (hInonfg : ¬I.FG) (n : ℕ)
    (g : ∀ m, m < n → I) :
    Set.Nonempty
      ((I : Set A) \
        Ideal.span (Finset.image (fun m : Fin n => (g m.1 m.2).1) Finset.univ : Set A)) :=
  by
  rw [Set.nonempty_iff_ne_empty]
  intro h
  rw [Set.diff_eq_empty] at h 
  apply hInonfg
  refine' ⟨Finset.image (fun m : Fin n => (g m.1 m.2).val) Finset.univ, _⟩
  refine' le_antisymm _ h
  rw [Ideal.span_le]
  intro a ha
  simp only [Fin.val_eq_coe, Subtype.val_eq_coe, Finset.coe_image, Finset.coe_univ, Set.image_univ,
    Set.mem_range] at ha 
...
noncomputable def f {R : Type} [CommRing R] {I : Ideal R[X]} [DecidableEq I] (hInonfg : ¬I.FG)
    (n : ℕ) : R[X] :=
  Nat.strongRecOn' n fun m hm =>
    Function.argminOn (Polynomial.natDegree : R[X] → ℕ) IsWellFounded.wf _ <| lemma1 hInonfg n sorry
