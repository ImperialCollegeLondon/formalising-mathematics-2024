...
#align_import solutions.section20representation_theory.sheet2
...
theorem comp_id (φ : RepMap ρ σ) : φ.comp (id ρ) = φ :=
  by
  ext
  rfl

theorem id_comp (φ : RepMap ρ σ) : (id σ).comp φ = φ :=
  by
  ext
  rfl
...
theorem comp_assoc {τ : Representation k G X} {Y : Type} [AddCommGroup Y] [Module k Y]
    ...
    (ξ.comp ψ).comp φ = ξ.comp (ψ.comp φ) := by rfl
