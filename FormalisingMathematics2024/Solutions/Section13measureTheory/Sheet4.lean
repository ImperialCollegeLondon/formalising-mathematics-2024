...
example (a : X) (s : Set Ω) : ∫ _ in s, a ∂μ = (μ s).toReal • a := by
  rw [integral_const]
  congr 2
...
example (s : Set Ω) (f g : Ω → ℝ) (hf : Measurable f) (hg : Measurable g)
  ...
  · convert measure_empty
    ...
    apply hx1
    apply hfg _ hx2
  · exact MeasurableSpace.measurableSet_compl _ _ <| measurableSet_eq_fun hf hg

example (f g h : Ω → ℝ)
    (h₁ : f ≤ᵐ[μ] g) (h₂ : f ≤ᵐ[μ] h) : 2 * f ≤ᵐ[μ] g + h := by
  convert EventuallyLE.add_le_add h₁ h₂ using 1
  rw [two_mul]
